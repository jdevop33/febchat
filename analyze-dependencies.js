...
const dependencyData = JSON.parse(fs.readFileSync('./dependency-analysis.json', 'utf8'));
...
function findCircularDependencies(data) {
  ...
  const uniqueCirculars = [...new Set(circulars.map(JSON.stringify))].map(JSON.parse);
...
function findComplexModules(data) {
  ...
  const complexityScores = Object.entries(data).map(([module, dependencies]) => ({
    module,
...
console.log('Modules that import many other modules (most dependencies first):\n');
...
console.log('Modules that are not imported by any other module (may be entry points or unused):\n');
results.potentiallyUnusedModules.forEach(module => {
  // Filter out some common entry points that are expected to be unused
  if (!module.includes('app/page') && !module.includes('app/layout') && !module.includes('scripts/')) {
    console.log(module);
  }
});

console.log(`\n\n4. COMPLEX MODULES\n${'-'.repeat(30)}`);
console.log('Modules with high complexity (many dependencies and imported by many other modules):\n');
results.complexModules.forEach(({ module, dependenciesCount, importedByCount, complexityScore }) => {
  console.log(`${module}: ${dependenciesCount} dependencies, imported by ${importedByCount} modules, score: ${complexityScore}`);
});

// Save detailed results to file
fs.writeFileSync('./dependency-analysis-results.json', JSON.stringify(results, null, 2));
