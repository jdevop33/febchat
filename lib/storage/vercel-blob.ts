...
export async function uploadPdf(
  ...
  if (!isProduction) {
    console.log(
      `[Dev] Skipping upload for bylaw ${bylawNumber} in development mode`,
    );
...
export async function uploadAllPdfs(): Promise<string[]> {
  ...
  console.log(
    `Successfully uploaded ${successUrls.length} of ${files.length} PDFs`,
  );
  return successUrls;
}
...
export async function deletePdf(bylawNumber: string): Promise<boolean> {
  if (!isProduction) {
    console.log(
      `[Dev] Skipping deletion for bylaw ${bylawNumber} in development mode`,
    );
    return true;
  }
