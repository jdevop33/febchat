...
export function getPineconeIndex() {
  try {
    ...
    connectionAttempts++;
    console.error(`Error connecting to Pinecone index (attempt ${connectionAttempts}/${MAX_RETRIES}):`, error);
    
    if (connectionAttempts < MAX_RETRIES) {
      console.log(`Retrying connection to Pinecone index (attempt ${connectionAttempts + 1}/${MAX_RETRIES})...`);
