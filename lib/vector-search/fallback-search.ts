...
export async function fallbackSearch(
  ...
  try {
    ...
    try {
      ...
      // Search with metadata filter only (no vector)
      ...
      if (searchResults.matches && searchResults.matches.length > 0) {
        console.log(`Found ${searchResults.matches.length} results with metadata-only search`);
      ...
      console.error('Metadata search failed, falling back to direct PDF search:', error);
    }
...
async function enhanceResultsWithKeywordScoring(
  ...
  const scoredResults = results.map(result => {
    ...
    const positionScore = matches > 0 ? 1 - (totalPosition / (text.length * matches)) : 0;
    ...
    for (const file of filesToProcess) {
      try {
        ...
        if (matches > 0) {
          ...
          const matchPos = content.toLowerCase().indexOf(keywords[0].toLowerCase());
