...
const issues = {
  importErrors: {
    regex: /Cannot find module|Unable to resolve path/,
    description: "Import path issues",
  },
  propTypeErrors: {
    regex:
      /Type .* is not assignable to type|Property .* does not exist on type/,
    description: "Prop type mismatches",
  },
  unusedImports: {
  ...
  missingComponents: {
    regex: /JSX element .* has no corresponding closing tag/,
    description: "Missing closing tags",
  },
  redundantCode: {
    regex: /Unreachable code detected/,
    description: "Unreachable code",
  },
  componentErrors: {
    regex:
      /(No overload matches this call|has no properties in common with type)/,
    description: "Component usage errors",
  },
};
  ...
  try {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (
        ...
        if (componentMatch) {
          ...
          if (
            (componentName === "MultimodalInput" ||
              componentName === "SuggestedActions" ||
              componentName === "PureStopButton" ||
              componentName === "PureSendButton" ||
    ...
    if (content.includes("memo(")) {
      ...
      memoComponents.forEach((memoComponent) => {
        ...
        const equalityFnMatch = content.match(
          new RegExp(`memo\\(${componentName}.*?\\{([\\s\\S]*?)\\}\\)`, "s"),
        );
          ...
          if (
            ...
            foundProblems.push({
              ...
              lineNumber:
                content
                  .split("\n")
                  .findIndex((line) => line.includes(`memo(${componentName}`)) +
                1,
              type: "memoInconsistency",
              description:
                "Memo component equality function missing prop check",
    ...
    const exportedComponents =
      content.match(/export const ([A-Za-z0-9]+)/g) || [];
    exportedComponents.forEach((exportMatch) => {
      ...
      if (
        ...
        if (
          content.includes(
            `const ${componentName} = memo(Pure${componentName}`,
          ) &&
          ...
          foundProblems.push({
            ...
            lineNumber:
              content
                .split("\n")
                .findIndex((line) =>
                  line.includes(`const ${componentName} = memo`),
                ) + 1,
...
if (foundProblems.length === 0) {
  ...
  foundProblems.forEach((problem, index) => {
    console.log(
      `Problem #${index + 1}: ${problem.description} in ${problem.file}:${problem.lineNumber}`,
    );
...
files.forEach((filePath) => {
  try {
    ...
    const componentDeclarations =
      content.match(/function ([A-Za-z0-9]+)\(/g) || [];
    ...
    console.error(
      `Error in component consistency check for ${filePath}:`,
      error,
    );
  }
});
...
const undeclaredComponents = Object.entries(allComponents).filter(
  ([name, info]) => !info.declaration && info.usages.length > 0,
);
...
if (undeclaredComponents.length > 0) {
  ...
  console.log(
    "All components used in the files appear to be properly declared.",
  );
}
...
files.forEach((filePath) => {
  try {
    ...
    const memoImplementations =
      content.match(/const ([A-Za-z0-9]+) = memo\(([A-Za-z0-9]+)/g) || [];
    ...
    memoImplementations.forEach((impl) => {
      ...
      if (!content.includes(`function ${pureComponentName}`)) {
        console.log(
          `In ${filePath}: ${componentName} memo wraps ${pureComponentName}, but ${pureComponentName} might not be defined`,
        );
      }

      // Check if both are used consistently
      const componentUsageCount = (
        content.match(new RegExp(`<${componentName}[ >]`, "g")) || []
      ).length;
      const pureComponentUsageCount = (
        content.match(new RegExp(`<${pureComponentName}[ >]`, "g")) || []
      ).length;

      if (pureComponentUsageCount > 0) {
        console.log(
          `Potential issue in ${filePath}: Both ${componentName} (${componentUsageCount} usages) and ${pureComponentName} (${pureComponentUsageCount} usages) are used in JSX. Usually only the memo-wrapped component should be used.`,
        );
      }
    });
  } catch (error) {
...
files.forEach((filePath) => {
  try {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      const propTypeMatch = line.match(
        /interface ([A-Za-z0-9]+Props)|type ([A-Za-z0-9]+Props)/,
      );
      if (propTypeMatch) {
        ...
        const hasDisabledProp =
          propsContent.includes("disabled?:") ||
          propsContent.includes("disabled:");

        // Find component JSX usage
        const componentUsages =
          content.match(new RegExp(`<${componentName}[\\s\\S]*?>`, "g")) || [];
        ...
        componentUsages.forEach((usage) => {
          ...
          if (usesDisabledProp && !hasDisabledProp) {
            console.log(
              `Potential issue in ${filePath}: ${componentName} uses disabled prop but it's not defined in ${propsName}`,
            );
          }
        });
      }
...
console.log(
  "1. Check memo component implementations - ensure Pure components are defined before being wrapped",
);
console.log(
  "2. Ensure disabled prop is consistently handled in all components that need it",
);
console.log(
  "3. Verify that memo equality functions check all necessary props, including disabled",
);
console.log("4. Update component props interfaces to include all used props");
console.log(
  "5. Check for any usage of components without passing required props",
);
