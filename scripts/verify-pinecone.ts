...
import { getPineconeClient, getPineconeIndex } from '../lib/vector-search/pinecone-client';
  ...
  try {
    ...
    if (searchResults.matches && searchResults.matches.length > 0) {
      ...
      if (firstResult.metadata) {
        console.log(`   - Bylaw: ${firstResult.metadata.bylawNumber || 'Unknown'}`);
    ...
    if (error instanceof Error) {
      ...
      if (error.message.includes('API key')) {
        ...
        console.error('- Check that your PINECONE_API_KEY is correct in .env.local');
        console.error('- Verify that your Pinecone account is active');
      } 
      else if (error.message.includes('index')) {
        console.error('\nTroubleshooting tips:');
        console.error('- Check that the index name "oak-bay-bylaws" exists in your Pinecone account');
        console.error('- Verify that the index is properly initialized with vectors');
        console.error('- Make sure your Pinecone account has access to the index');
      }
      else if (error.message.includes('timeout') || error.message.includes('ECONNREFUSED')) {
        console.error('\nTroubleshooting tips:');
        console.error('- Check your network connection');
        console.error('- Verify the Pinecone service is running and accessible');
        console.error('- Make sure your firewall allows connections to Pinecone');
      }
    }
