...
async function testVercelIntegrations() {
  ...
  try {
    ...
    if (!databaseUrl) {
      recordTest('Database URL', false, 'No database URL found in environment variables');
    } else {
    ...
    recordTest('PostgreSQL', false, `Failed to connect: ${(error as Error).message}`);
  }
  ...
  try {
    ...
    if (!blobToken) {
      recordTest('Blob Storage', false, 'No BLOB_READ_WRITE_TOKEN found in environment variables');
    } else {
    ...
    recordTest('Blob Storage', false, `Failed to access blob storage: ${(error as Error).message}`);
  }
  ...
  try {
    ...
    recordTest('Pinecone', false, `Failed to connect to Pinecone: ${(error as Error).message}`);
  }
  ...
  if (!anthropicKey) {
    recordTest('Anthropic API', false, 'No ANTHROPIC_API_KEY found in environment variables');
  } else if (anthropic_key_valid(anthropicKey)) {
    recordTest('Anthropic API', true, 'ANTHROPIC_API_KEY found and has valid format');
  } else {
    recordTest('Anthropic API', false, 'ANTHROPIC_API_KEY has invalid format');
  }
  
  if (!openaiKey) {
    recordTest('OpenAI API', false, 'No OPENAI_API_KEY found in environment variables');
  ...
  if (!authSecret) {
    recordTest('Auth Secret', false, 'No AUTH_SECRET or NEXTAUTH_SECRET found in environment variables');
  } else if (authSecret.length < 16) {
    recordTest('Auth Secret', false, 'Auth secret is too short (should be at least 16 characters)');
  } else {
    recordTest('Auth Secret', true, 'Auth secret is properly configured');
  }
  
  if (!authUrl) {
    recordTest('Auth URL', false, 'No NEXTAUTH_URL found in environment variables');
...
function openai_key_valid(key: string): boolean {
  return (key.startsWith('sk-') && key.length > 20) || key.startsWith('sk-org-');
}
...
function printSummary() {
  ...
  console.log(`${colors.bold}Results: ${passed}/${total} tests passed (${failed} failed)${colors.reset}\n`);
  
  if (failed > 0) {
    log.warning('Failed Tests:');
    results.filter(r => !r.passed).forEach(result => {
      console.log(`${colors.red}✗ ${result.name}: ${result.message}${colors.reset}`);
    });
