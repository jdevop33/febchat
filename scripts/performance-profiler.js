...
function injectProfilingCode(filePath) {
  ...
  const modifiedProfilingCode = PROFILING_CODE.replace(/Component\.name/g, `'${componentName}'`);
  ...
  for (let i = insertionLine; i < lines.length; i++) {
    if (lines[i].includes('return (') || 
        ...
        (lines[i].trim() === 'return (' || lines[i].trim() === '{')) {
...
console.log('You may want to remove the profiling code before production deployment.');