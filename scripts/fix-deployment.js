...
function checkVercelConfig() {
  ...
  try {
    ...
    if (vercelConfig.env.NEXTAUTH_URL !== CORRECT_DOMAIN) {
      console.log(
        `⚠️ Incorrect NEXTAUTH_URL: ${vercelConfig.env.NEXTAUTH_URL || "missing"}`,
      );
    ...
    if (vercelConfig.buildCommand?.includes("build-with-db.sh")) {
      console.log(
        "⚠️ Found potentially problematic build command using shell script",
      );
...
function checkPackageJson() {
  ...
  try {
    ...
    if (packageJson.scripts.build?.includes("build-with-db.sh")) {
      console.log(
        "⚠️ Found potentially problematic build script in package.json",
      );
...
function checkEnvFile() {
  ...
  console.log(
    `✅ Updated ${path.basename(targetPath)} with correct configuration`,
  );
}
