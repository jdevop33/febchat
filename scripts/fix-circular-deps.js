...
const SHARED_TYPES_PATH = path.join(process.cwd(), 'types/shared/shared-types.ts');
...
function extractTypes(filePath) {
  const content = fs.readFileSync(filePath, 'utf8');
  const typeRegex = /export\s+(type|interface)\s+([A-Za-z0-9_]+)(?:\s*=\s*[^{;]+|\s*{([^}]*)}\s*)/g;
  const types = [];
...
function fixImports(filePath, circularImports) {
  ...
  circularImports.forEach(importInfo => {
    if (importInfo.type === 'named') {
      const importPattern = new RegExp(`import\\s+{[^}]*${importInfo.names.join('[^}]*,\\s*[^}]*')}[^}]*}\\s+from\\s+['"]${importInfo.path.replace(/\//g, '\\/')}['"]`, 'g');
      modifiedContent = modifiedContent.replace(importPattern, `import { ${importInfo.names.join(', ')} } from '@/types/shared/shared-types'`);
    }
  });
...
function updateSharedTypes(extractedTypes) {
  ...
  extractedTypes.forEach(typeInfo => {
    if (!existingTypes.has(typeInfo.name)) {
      const typeDefinition = typeInfo.kind === 'type'
  ...
  if (typesAdded > 0) {
    ...
    console.log(`Added ${typesAdded} new types to shared-types.ts (saved as shared-types.ts.updated)`);
  } else {
...
CIRCULAR_COMPONENTS.forEach(componentPath => {
  ...
  if (fs.existsSync(fullPath)) {
    ...
    const circularImports = imports.filter(imp => {
      const importPath = imp.path.startsWith('@/') ? imp.path.substring(2) : imp.path;
      return CIRCULAR_COMPONENTS.some(cp => {
        const componentPath = cp.startsWith('./') ? cp.substring(2) : cp;
        return importPath.includes(componentPath) || importPath === componentPath;
      });
    });
    
    if (circularImports.length > 0) {
      console.log(`Found ${circularImports.length} circular imports in ${componentPath}`);
...
console.log('Review the .fixed and .updated files, and if they look good, replace the originals.');