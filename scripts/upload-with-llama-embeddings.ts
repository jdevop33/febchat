    ...
    return texts.map((text) => {
      ...
      return Array.from({ length: EMBEDDING_DIMENSIONS }, (_, i) => {
        const value = Number.parseInt(
          hash.substring(i % hash.length, (i % hash.length) + 2),
          16,
        );
  ...
  for (const pattern of patterns) {
    ...
    if (match) {
      ...
      if (pattern.titleIdx && match[pattern.titleIdx]) {
        result.title = pattern.titleFn
          ? pattern.titleFn(match[pattern.titleIdx])
          : match[pattern.titleIdx];
      }
      return result;
    }
  }
...
function extractSection(text: string): string {
  ...
  const sectionMatch = text.match(
    /(?:section|part|division)\s+([IVXivx\d]+(?:\.\d+)?)/i,
  );
  if (sectionMatch) {
...
async function upsertToPinecone(vectors: any[]): Promise<void> {
  ...
  console.log(
    `Upserting ${vectors.length} vectors to Pinecone in batches of ${upsertBatchSize}...`,
  );
  ...
  for (let i = 0; i < vectors.length; i += upsertBatchSize) {
    ...
    console.log(
      `Upserted batch ${Math.floor(i / upsertBatchSize) + 1}/${Math.ceil(vectors.length / upsertBatchSize)}`,
    );
  }
}
