...
const endpoints = [
  ...
  { path: '/api/chat', expectedStatus: 401, description: 'Chat API (should require auth)' },
  { path: '/api/auth/csrf', expectedStatus: 200, description: 'CSRF protection endpoint' },
];
...
function checkDomain(target) {
  ...
  if (domain === 'app.fitforgov.com') {
    console.log(`${colors.green}✓ Using production domain: ${domain}${colors.reset}`);
    return true;
  } else if (domain.includes('vercel.app')) {
    console.log(`${colors.yellow}⚠ Using Vercel preview domain: ${domain}${colors.reset}`);
...
async function checkAuthRedirects() {
  ...
  if (response.body.includes('juche.org')) {
    ...
    console.log(`${colors.green}✓ No invalid domain redirects detected${colors.reset}`);
    return true;
  }
}
...
function checkSecurityHeaders(headers) {
  ...
  for (const [key, expectedValue] of Object.entries(expectedHeaders)) {
    ...
    if (!actualValue) {
      ...
      result.incorrect.push(`${key}: expected "${expectedValue}", got "${actualValue}"`);
      result.pass = false;
    }
  }
...
async function main() {
  ...
  if (!domainOk) {
    console.log(`${colors.yellow}⚠ Domain check failed, but continuing with other checks...${colors.reset}`);
  }
  ...
  for (const endpoint of endpoints) {
    ...
    if (response.success) {
      ...
      console.log(`${colors.red}✗ ${response.status || response.error}${colors.reset}`);
  ...
  if (securityHeaders) {
    ...
    if (headerCheck.pass) {
      console.log(`${colors.green}✓ All required security headers are present${colors.reset}`);
    } else {
      if (headerCheck.missing.length > 0) {
        console.log(`${colors.red}✗ Missing headers: ${headerCheck.missing.join(', ')}${colors.reset}`);
      }
  ...
  if (allEndpointsOk && authRedirectsOk && domainOk) {
    console.log(`${colors.green}✅ PASSED: Deployment appears to be healthy!${colors.reset}`);
  } else {
    console.log(`${colors.yellow}⚠️ CAUTION: Some checks failed, deployment may have issues.${colors.reset}`);
    ...
    if (!domainOk) {
      ...
      console.log(`- Update NEXTAUTH_URL in environment variables to match the domain`);
    }
    ...
    if (!authRedirectsOk) {
      console.log(`- Verify that middleware.ts is correctly filtering callback URLs`);
