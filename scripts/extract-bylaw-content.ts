...
async function extractBylawContent() {
  try {
    ...
    const section3Match = text.match(/3\s+\([1-9]\)[^\n]*(?:\n[^\n]+)*?(?=\([2-9]\)|\n\s*4\s+)/s);
    if (section3Match) {
    ...
    const section4Match = text.match(/4\s+No person shall[^\n]*(?:\n[^\n]+)*?(?=\n\s*5\s+The provisions)/s);
    if (section4Match) {
      console.log(section4Match[0].trim());
    } else {
      console.log('Section 4 not found - this contains the construction noise regulations.');
      
      // Try to find section 4 manually by looking for key elements
    if (text.includes("No person shall") && text.includes("operate, use or permit to be operated or used, the following:")) {
      ...
      for (const line of lines) {
        ...
        if (inSection4 && line.trim().startsWith('5') && line.includes("provisions of this Bylaw shall not apply")) {
          inSection4 = false;
          break;
        }
      }
    ...
    const section5Match = text.match(/5\s+The provisions of this Bylaw shall not apply[^6]+/s);
    if (section5Match) {
