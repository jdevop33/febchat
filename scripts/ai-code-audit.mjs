...
async function runCodeImprovement(outputDir) {
  ...
  if (argv.focus) {
    ...
    filesToImprove = filesToImprove.filter(
      (file) =>
...
function printSummary(results) {
  ...
  if (results.improve.length > 0) {
    ...
    if (failCount === 0) {
      console.log(
        chalk.green(
          `✅ Code Improvement: ${successCount}/${results.improve.length} files improved`,
        ),
      );
    } else {
      console.log(
        chalk.yellow(
          `⚠️ Code Improvement: ${successCount}/${results.improve.length} files improved, ${failCount} failed`,
        ),
      );
