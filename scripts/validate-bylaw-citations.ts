 ...
 */
async function validateBylawCitation(bylawNumber: string, section: string, content: string): Promise<boolean> {
  ...
  if (!fs.existsSync(pdfPath)) {
    console.warn(`⚠️ PDF file not found for bylaw ${bylawNumber} at path ${pdfPath}`);
    return false;
  }
  ...
  const sectionPattern = new RegExp(`\\b${section.replace(/[()]/g, '\\$&')}\\b`, 'i');
...
async function validateMockBylawData() {
  ...
  for (const [index, item] of mockBylawData.entries()) {
    ...
    if (bylawNumber && section && content) {
      ...
      const isValid = await validateBylawCitation(bylawNumber, section, content);
...
async function validateDatabaseBylawData() {
  ...
  const sections = await prisma.bylawSection.findMany({
    include: { bylaw: true }
  }).catch(err => {
  ...
  for (const [index, section] of sections.entries()) {
    ...
    const isValid = await validateBylawCitation(bylawNumber, sectionNumber, content);
    
    if (isValid) {
      console.log(`✅ Valid: Bylaw ${bylawNumber}, Section ${sectionNumber}`);
    } else {
      console.error(`❌ INVALID: Bylaw ${bylawNumber}, Section ${sectionNumber}`);
      console.error(`Content: "${content.substring(0, 100)}..."`);
    }
  }
}
