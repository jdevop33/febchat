    ...
    importPaths.forEach(importPath => {
      ...
      for (const ext of extensions) {
        ...
        if (fs.existsSync(fullPath)) {
          console.log(`✅ Import ${importPath} -> Found at ${resolvedPath}${ext}`);
          found = true;
          break;
        }
      }
      ...
      if (!found) {
        for (const ext of extensions) {
          ...
          if (fs.existsSync(indexPath)) {
            console.log(`✅ Import ${importPath} -> Found at ${resolvedPath}/index${ext}`);
            found = true;
            break;
          }
        }
      ...
      if (!found) {
        console.log(`❌ Import ${importPath} -> Not found at ${resolvedPath}.*`);
        ...
        try {
          if (fs.existsSync(path.join(rootDir, dirPath))) {
            ...
            const similarFiles = dirContents.filter(file => file.startsWith(fileName || ''));
...
ensureJsConfig();
console.log('\n💡 Fix Recommendations:');
console.log('1. Update .eslintrc.json to ignore import issues or configure resolver correctly');
console.log('2. Ensure all imported files actually exist at the expected locations');
console.log('3. Try using relative imports instead of path aliases for problematic imports'); 