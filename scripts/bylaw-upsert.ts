...
// Configuration
const PDF_DIRECTORY = process.argv[2] || path.resolve(process.cwd(), 'public', 'pdfs');
...
function extractBylawInfo(filename: string): {
  ...
  const consolidatedMatch = basename.match(/CONSOLIDATED|Consolidated|consolidated/);
...
function extractAmendmentReferences(text: string): { 
  ...
  const enactmentRegex = /(?:enacted|adopted|passed)(?:\s+on)?\s+(?:the\s+)?(\d{1,2}(?:st|nd|rd|th)?[\s,]+(?:day\s+of\s+)?[A-Za-z]+[\s,]+\d{4})/i;
  ...
  const amendmentDateRegex = /(?:last\s+amended|amended\s+on)(?:\s+the\s+)?(\d{1,2}(?:st|nd|rd|th)?[\s,]+(?:day\s+of\s+)?[A-Za-z]+[\s,]+\d{4})/i;
 ...
 */
function extractSectionInfo(text: string): { section?: string, title?: string } {
 ...
 */
function chunkText(text: string, chunkSize: number, chunkOverlap: number): { pageContent: string, metadata: any }[] {
...
async function processPDF(filePath: string): Promise<{
  ...
  try {
    ...
    console.log(`Created ${chunks.length} chunks with size ${CHUNK_SIZE} and overlap ${CHUNK_OVERLAP}`);
    ...
    console.log(`Preparing to generate ${texts.length} embeddings in batches of ${EMBEDDING_BATCH_SIZE}...`);
    
    for (let i = 0; i < texts.length; i += EMBEDDING_BATCH_SIZE) {
      console.log(`Processing batch ${Math.floor(i / EMBEDDING_BATCH_SIZE) + 1}/${Math.ceil(texts.length / EMBEDDING_BATCH_SIZE)}`);
      ...
      while (!success && retryCount <= maxRetries) {
        try {
          ...
          if (retryCount <= maxRetries) {
            console.warn(`Retrying batch ${batchNumber} after ${waitTime}ms (attempt ${retryCount}/${maxRetries})...`);
            await new Promise(resolve => setTimeout(resolve, waitTime));
          } else {
            console.error(`❌ Failed to upsert batch ${batchNumber} after ${maxRetries} attempts:`, error);
            throw error;
          }
        }
      }
...
async function main(): Promise<void> {
  try {
    ...
    console.log(`Using Pinecone index: ${process.env.PINECONE_INDEX || 'oak-bay-bylaws-v2'}`);
    console.log(`Using embedding provider: ${process.env.EMBEDDING_PROVIDER || 'llamaindex'}`);
