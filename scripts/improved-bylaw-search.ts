...
async function searchBylaws() {
  try {
    ...
    if (searchResults.matches && searchResults.matches.length > 0) {
      ...
      const bylawResults = new Map<string, {
        bylawNumber: string;
        title: string;
      ...
      const sortedResults = Array.from(bylawResults.values())
...
function extractRelevantText(text: string, query: string): string {
  ...
  if (topSentences.length > 0 && topSentences[0].score > 0) {
    ...
    const highlightedSnippet = contextSnippet.replace(pattern, match => match);
...
if (query) {
  ...
  console.log('Usage: pnpm tsx scripts/improved-bylaw-search.ts <query> [--category=<category>] [--bylaw=<bylawNum>]');
  process.exit(1);
}