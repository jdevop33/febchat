...
export async function saveChat({
  ...
  title = !title || typeof title !== 'string' || title === '' ? 'New Chat' : title.length > 255 ? `${title.substring(0, 252)}...` : title;
  visibility = visibility !== 'public' && visibility !== 'private' ? 'private' : visibility;
...
export async function voteMessage({
  ...
  try {
    ...
    const messageExists = (await db.select().from(message).where(and(eq(message.id, messageId), eq(message.chatId, chatId))))[0];
    if (!messageExists) {
      throw new Error('Message not found in chat');
    }

    const existingVote = (await db.select().from(vote).where(and(eq(vote.messageId, messageId), eq(vote.chatId, chatId))))[0];
    if (existingVote) {
...
export async function getDocumentById(id: string): Promise<unknown> {
  try {
    return (await db.select().from(document).where(eq(document.id, id)).orderBy(desc(document.createdAt)))[0];
  } catch (error) {
...
export async function saveSuggestions(suggestions: schema.Suggestion[]): Promise<void> {
...
export async function getSuggestionsByDocumentId(documentId: string): Promise<unknown[]> {
  try {
